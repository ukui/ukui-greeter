      ...
      m_activeScreen(0)
{
    ...
    connect(m_model.data(), SIGNAL(modelReset()), this, SLOT(onScreenCountChanged()));
...
void MainWindow::paintEvent(QPaintEvent *e)
{
    ...
    for(int i = 0; i < m_model->rowCount(); i++){
        ...
        QRect logoRect(rect.left() + 50, rect.bottom()-120, m_logo.width(), m_logo.height());
        painter.drawPixmap(logoRect, m_logo);

        //在没有登录窗口的屏幕上显示图标
        if(i != m_activeScreen)
            painter.drawPixmap((rect.left()+rect.width()-cof.width())/2, (rect.top()+rect.height()-cof.height())/2, cof.width(), cof.height(), cof);
    }
